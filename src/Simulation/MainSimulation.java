        ...
        while (car.getPositionOnRoad() < 4 && road.getRoadId() <= 2) {
            ...
            if (car.getPositionOnRoad() == 4 && road.getRoadId() == 1) {
                ...
                while (trafficLight.getCurrentState().equals("Red") && car.getPositionOnRoad() == 4 && road.getRoadId() == 1) {
                    ...
                    System.out.println("Traffic lights are changing");
                    trafficLight.operate();

                    System.out.println("Traffic lights are now : " + trafficLight.getCurrentState());//operates lights if current state is red and position is 4 and roadId is 1
                    if (trafficLight.getCurrentState().equals("Green") && car.getPositionOnRoad() == 4 && road.getRoadId() == 1) {
                        road.roadChange();
                        car.setPositionOnRoad(0);
                        System.out.println("Car " + car.getId() + " is now travelling on road : " + road.getRoadId() + " on segment " + car.getPositionOnRoad());
                    } else if (trafficLight.getCurrentState().equals("Red") && car.getPositionOnRoad() == 4 && road.getRoadId() == 1) {
                        trafficLight.operate();
                        road.setRoadId(1);
